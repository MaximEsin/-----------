...
import { CustomEventProps, Spin4DeadEventTypes, Spin4DeadRefreshEvent } from '../../../../interfaces/events';
...
const Refresh: FC<CustomEventProps> = ({ eventData, onSubmit }) => {
  ...
  useEffect(() => {
    const checkEventData = () => {
      if (eventData) {
        const { type, symbols } = eventData as Spin4DeadRefreshEvent;
        if (type === Spin4DeadEventTypes.Refresh) {
  ...
  const handleSubmit = () => {
    onSubmit({ type: Spin4DeadEventTypes.Refresh, symbols: reelsState, toReplace: reelsState });
